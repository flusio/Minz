...
class Form
{
     ...
     * Initialize a form.
     *
     * You can overrides default values with the default_values parameter.
     *
     * You can bind a model to the form so its properties are synchronized with
     * the form fields (see Form\Field::$bind_model).
     *
    ...
    public function fieldsSchema(): array
    {
        ...
        foreach ($properties as $property) {
            ...
            if ($field_type === 'string') {
                $field_schema['trim'] = $field_attribute->trim;
            } else {
                $field_schema['trim'] = false;
            }
            ...
            $field_schema['bind_model'] = $field_attribute->bind_model;

            $fields_schema[$property_name] = $field_schema;
        }
     ...
     *
     * If the field is declared with the `trim: true` option and that the value
     * is a string, it's automatically trimed.
     *
     * If the form has been bounded to a model, its properties are synchronized
     * (see the field `bind_model` attribute).
     *
     * @throws Errors\LogicException
    ...
    public function set(string $field_name, mixed $value): void
    {
        ...
        if ($field_schema['trim'] && is_string($value)) {
            $value = trim($value);
        }

        $this->$field_name = $value;

        $bind_model = $field_schema['bind_model'];
    ...
    public function checkModel(): void
    {
        ...
        foreach ($fields_schema as $field_name => $field_schema) {
            if ($field_schema['bind_model'] === false) {
                continue;
            }

    ...
    public function handleRequest(Request $request): void
    {
        ...
        foreach ($fields_schema as $field_name => $field_schema) {
            if ($field_schema['type'] === 'bool') {
                $value = $request->params->getBoolean($field_name);
            } elseif ($field_schema['type'] === 'int') {
                $value = $request->params->getInteger($field_name);
            } elseif ($field_schema['type'] === 'DateTimeImmutable') {
                $value = $request->params->getDatetime($field_name, format: $field_schema['format']);
            } elseif ($field_schema['type'] === 'array') {
                $value = $request->params->getArray($field_name);
            } else {
                $value = $request->params->getString($field_name);
            }

            if ($value === null && !$request->params->has($field_name)) {
                continue;
            }

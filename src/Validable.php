 ...
 *                 $this->addError('@base', 'Credentials are invalid.');
 *             }
 *         }
 *     }
 *
...
trait Validable
{
     ...
     *
     * The values are an array with two elements: the first element is the
     * error code, the second element is the error message.
     *
    ...
    public function errors(bool $format = true): array
    {
        ...
        foreach ($this->errors as $field_name => $field_errors) {
            $messages = array_column($field_errors, 1);
            $formatted_errors[$field_name] = implode(' ', $messages);
        }
     ...
     *
     * The error code is generally a PropertyCheck class name, or the Check
     * method name.
     */
